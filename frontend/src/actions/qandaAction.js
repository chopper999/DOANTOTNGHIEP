...
// create Dataset
export const createDataset = (tag, patterns, responses) => async(dispatch) => {
    const dataAPI = {
        tag: tag,
        patterns: patterns,
        responses: responses
    ...
    try {
        ...
        const {data} = await Axios.post(`https://quocdatit.tk/dataset/create-dataset/`, dataAPI);
        if (data){
            dispatch({type: DATASET_CREATE_SUCCESS, payload: data});
        }
    } catch (error) {
        const message =
      error.response && error.response.data.message
        ? error.response.data.message
        : error.message;
    dispatch({ type: DATASET_CREATE_FAIL, payload: message });
    }
}