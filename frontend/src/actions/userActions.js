...
export const updateUserProfile = (user) => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch({type: USER_SIGNIN_SUCCESS, payload: data}); //cap nhat lai trang thai dang nhap
    localStorage.setItem('userInfo', JSON.stringify(data)); //luu lai trong localStorage
  } catch (error) {
    const message =
      error.response && error.response.data.message
        ? error.response.data.message
